...
It is suggseted to use this when you want to create a BIM viewer and you don't want to spend time implementing all the model navigation tools you would like to have.
...
Below code is where we are setting up a basic threejs scene using `IfcViewerAPI()` methos of `web-ifc-viewer`. Note that this code is just the viewer - In the next step we will add the functionality to load models into this viewer.
...
// load model from the below path in your respository
