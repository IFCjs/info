...
```js
const properties = await IfcViewerAPI.IFC.getProperties(modelID,
                                                         id, true)
```
...
```js
const modelID = await IfcViewerAPI.IFC.ifcModel.modelID
const pickID = await IfcViewerAPI.IFC.selector.pickIfcItemsByID(
          modelID,
          0,
          true)
```
