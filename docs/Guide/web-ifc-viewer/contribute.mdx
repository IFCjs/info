...
<IfcAlert>To run the application locally we will need a local server. If you're using VS Code as your IDE, one option is to install the Live Server extension, which allows us to open an instance of Google Chrome, run our web application, and see the changes we make to the code in real time.</IfcAlert>
...
🚴‍♂️🚴‍♂️
The `setGrid` function allows us to set: `size`, `divisions`, `colorCenterLine` and colorGrid`. It is possible to create a custom grid.
...
<IfcCard icon="🏆" title="Congratulations">
Thanks to your contribution we can make the library more extensive and able to meet the needs of developers and architects.
</IfcCard>
