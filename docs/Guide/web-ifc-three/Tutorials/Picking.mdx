...
- The threeCanvas object is the HTML <code>&lt;canvas&gt;</code> element where the Three.js scene is being rendered. A reference to it can be retrieved with `getElementByID()`.
